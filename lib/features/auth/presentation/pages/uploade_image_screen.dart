    ...
    if (source == ImageSource.camera) {
      ...
      if (!status.isGranted) {
        if (status.isPermanentlyDenied) {
          await _showPermissionDialog(
            'Kamera ruxsati',
            'Kameradan foydalanish uchun sozlamalarda ruxsat bering.',
            true,
          );
        }
        return false;
      ...
      if (!status.isGranted) {
        if (status.isPermanentlyDenied) {
          await _showPermissionDialog(
            'Galereya ruxsati',
            'Galereyadan foydalanish uchun sozlamalarda ruxsat bering.',
            true,
          );
        }
        return false;
  ...
  Future<void> _pickImage(ImageSource source) async {
    try {
      ...
      if (e.toString().contains('permission') ||
        ...
        errorMessage =
            'Kamera ishlamayapti. Qurilmada kamera mavjudligini tekshiring.';
      } else if (e.toString().contains('gallery') ||
          e.toString().contains('Gallery')) {
  ...
  Widget _buildNextButton() {
    return Padding(
      ...
      child: WScaleAnimation(
        onTap:
            (image != null && base64Image != null && !isProcessing)
                ? () {
